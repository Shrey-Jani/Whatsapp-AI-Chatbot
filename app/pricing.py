...
def estimate(service: str, answers: dict, prices: dict = PRICING) -> str:
    ...
    if service == "Business Registration":
        ...
        line = f"Estimated fee: Company Incorporation - ${p['flat']} flat ({p['breakdown']})."
        if answers.get("company_type") == "Named":     # card p18: named companies vary
            line += " A named company adds a name-search (NUANS) fee, which varies by name."
        return line
